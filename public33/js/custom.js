...
var CawCart = function(){
	...
	var handleShopPannel = function(){
		...
		if(screenWidth >= 1199 ){
			// $("#shopFilter").slideToggle("slow"); // enable this line later to show filter on page load
...
function renderList(tempdata) {
			  ...
			  const listItems = tempdata.map(item => `
...
const existingCookie = document.cookie.match('currency=([^;]+)');

// If the cookie doesn't exist, set the default cookie with the domain attribute
if (!existingCookie) {
  document.cookie = 'currency=pound; domain=.cawgallery.co.uk; path=/'; // Replace yourdomain.com with your actual domain
} else {
  // Get the selected currency from the cookie
  const selectedCurrency = existingCookie[1];

  // Set the selected option in the select element
  selectElement.value = selectedCurrency;
}

// Function to set the currency cookie and reload the page
function setCurrencyCookie(currency) {
  document.cookie = `currency=${currency}; domain=.cawgallery.co.uk; path=/`; // Replace yourdomain.com with your actual domain
